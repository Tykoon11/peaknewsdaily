    ...
    <div className="space-y-6">
      ...
      <div className="bg-white dark:bg-gray-800 rounded-xl p-6 shadow-lg border border-slate-200 dark:border-gray-700">
        ...
        <div className="mt-4 flex items-center justify-between text-sm text-gray-600 dark:text-gray-400">
          <span>
            Showing {filteredNews.length} of {initialNews.length} articles
            {searchTerm && (
            ...
            {selectedTopic !== 'all' && (
              <span className="ml-2 px-2 py-1 bg-green-100 dark:bg-green-900 text-green-800 dark:text-green-200 rounded capitalize">
            ...
            >
              <div className="flex flex-col sm:flex-row sm:items-center gap-2 sm:gap-3 text-xs sm:text-sm text-slate-500 dark:text-gray-400 mb-3">
                <Link
                  ...
                  className="px-3 py-1 bg-gradient-to-r from-blue-500 to-indigo-600 text-white rounded-full hover:from-blue-600 hover:to-indigo-700 transition-colors capitalize font-medium w-fit"
                >
                  {item.topic.title}
                </Link>
              ...
              <Link href={`/news/${item.slug}`} className="block">
                <h3 className="text-lg sm:text-xl font-bold text-slate-800 dark:text-gray-100 group-hover:text-blue-600 dark:group-hover:text-blue-400 transition-colors mb-3 leading-tight">
                  {item.title}
                </h3>
                {item.excerpt && (
