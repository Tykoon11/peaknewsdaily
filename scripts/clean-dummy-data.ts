    ...
    const deletedPosts = await prisma.post.deleteMany({
      where: {
        OR: [
          ...
          { slug: { contains: 'ivyj' } },
          { slug: { contains: 'guhu' } },
          // Keep only trading/investing content
          { AND: [
            { title: { not: { contains: 'Trading' } } },
            { title: { not: { contains: 'Crypto' } } },
            { title: { not: { contains: 'Position' } } },
            { title: { not: { contains: 'Wallet' } } },
            { title: { not: { contains: 'Security' } } },
            { title: { not: { contains: 'Investment' } } },
            { title: { not: { contains: 'Market' } } },
            { title: { not: { contains: 'Finance' } } }
          ]}
        ]
      }
    })
