  ...
  return (
    <>
      ...
      <section className="relative overflow-hidden bg-gradient-to-br from-slate-900 via-indigo-900 to-purple-900 dark:from-gray-900 dark:via-slate-800 dark:to-gray-900">
        ...
        <div className="relative container mx-auto px-3 xs:px-4 py-8 xs:py-10 sm:py-12 md:py-16 lg:py-20">
          <div className="max-w-4xl mx-auto text-center">
            {/* Breadcrumb */}
            <nav className="flex flex-wrap items-center justify-center gap-1 xs:gap-2 text-xs xs:text-sm sm:text-base text-indigo-200/80 mb-4 xs:mb-6 sm:mb-8 px-2 xs:px-0">
            ...
            {topic.description && (
              <p className="text-sm xs:text-base sm:text-lg md:text-xl lg:text-2xl text-indigo-100/80 mb-4 xs:mb-6 sm:mb-8 max-w-3xl mx-auto leading-relaxed px-3 xs:px-4 sm:px-6 lg:px-0">
                {topic.description}
              </p>
            )}
            ...
            <div className="grid grid-cols-1 xs:grid-cols-3 gap-3 xs:gap-4 sm:gap-6 mt-6 xs:mt-8 sm:mt-12 px-2 xs:px-0">
              ...
              <div className="relative bg-white/10 backdrop-blur-lg rounded-lg xs:rounded-xl sm:rounded-2xl p-3 xs:p-4 sm:p-6 border border-white/20 shadow-2xl shadow-purple-500/10">
                <div className="flex items-center gap-2 mb-1">
                  ...
                  <div className="text-lg xs:text-xl sm:text-2xl md:text-3xl font-black text-green-400">LIVE</div>
                </div>
                <div className="text-indigo-200/80 font-medium text-xs sm:text-sm leading-tight">Updates</div>
              </div>
            </div>
          </div>
        </div>
      ...
      <main className="bg-gradient-to-b from-slate-50 to-white dark:from-gray-900 dark:to-gray-800">
        <div className="container mx-auto px-3 xs:px-4 py-6 xs:py-8 sm:py-12 lg:py-16">
          <div className="grid gap-6 xs:gap-8 lg:grid-cols-3">
            
            {/* Articles List */}
            <div className="lg:col-span-2">
              {newsItems.length === 0 ? (
                <div className="text-center py-8 xs:py-12 sm:py-16 bg-white dark:bg-gray-800 rounded-lg xs:rounded-xl sm:rounded-2xl shadow-lg dark:shadow-2xl border border-slate-200 dark:border-gray-700 mx-2 xs:mx-0">
                  <div className="p-4 xs:p-6 sm:p-8 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-full w-12 h-12 xs:w-16 xs:h-16 sm:w-20 sm:h-20 mx-auto mb-4 xs:mb-6 flex items-center justify-center">
                    <svg className="w-6 h-6 xs:w-8 xs:h-8 sm:w-10 sm:h-10 text-white" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                      <path strokeLinecap="round" strokeLinejoin="round" strokeWidth={2} d="M19 20H5a2 2 0 01-2-2V6a2 2 0 012-2h10a2 2 0 012 2v1m2 13a2 2 0 01-2-2V7m2 13a2 2 0 002-2V9.5a2.5 2.5 0 00-2.5-2.5H15" />
                    </svg>
                  </div>
                  <h3 className="text-lg xs:text-xl sm:text-2xl font-bold text-slate-800 dark:text-gray-100 mb-2 px-2 xs:px-0">No Articles Yet</h3>
                  <p className="text-slate-600 dark:text-gray-300 text-sm xs:text-base px-4 xs:px-6 sm:px-0">
                ...
                <div className="space-y-3 xs:space-y-4 sm:space-y-6">
                  {newsItems.map((item, index) => (
                    <article
                      key={item.id}
                      className="group bg-white dark:bg-gray-800 rounded-lg xs:rounded-xl sm:rounded-2xl border border-slate-200 dark:border-gray-700 overflow-hidden shadow-lg dark:shadow-2xl hover:shadow-xl dark:hover:shadow-2xl transition-all duration-300 transform hover:-translate-y-1 mx-2 xs:mx-0"
                    >
                      ...
                      <div className="p-3 xs:p-4 sm:p-6">
                        ...
                        <Link href={`/news/${item.slug}`} className="block">
                          <h2 className={`font-bold text-slate-800 dark:text-gray-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400 transition-colors mb-2 xs:mb-2 sm:mb-3 leading-tight ${
                            index === 0 ? 'text-lg xs:text-xl sm:text-2xl' : 'text-base xs:text-lg sm:text-xl'
                          }`}>
                            {item.title}
                          </h2>
                          {item.excerpt && (
                            <p className="text-slate-600 dark:text-gray-300 text-xs xs:text-sm sm:text-base line-clamp-3 leading-relaxed">
                              {item.excerpt}
                            </p>
                          )}
                        </Link>
            ...
            {/* Sidebar */}
            <aside className="space-y-4 xs:space-y-6 sm:space-y-8">
              <div className="bg-gradient-to-br from-indigo-50 to-purple-50 dark:from-gray-800 dark:to-gray-700 rounded-lg xs:rounded-xl sm:rounded-2xl p-4 xs:p-6 sm:p-8 border border-indigo-100/50 dark:border-gray-600 shadow-lg dark:shadow-2xl mx-2 xs:mx-0">
                <div className="flex items-center gap-2 xs:gap-3 mb-4 xs:mb-6">
                  <div className="p-1.5 xs:p-2 bg-gradient-to-r from-indigo-500 to-purple-600 rounded-md xs:rounded-lg shadow-lg">
                  ...
                  <h3 className="text-lg xs:text-xl font-bold text-slate-800 dark:text-gray-100">All Topics</h3>
                </div>
                ...
                <div className="space-y-1.5 xs:space-y-2">
                  {allTopics.map((t) => (
                    <Link
                      ...
                      className={`block p-2.5 xs:p-3 sm:p-4 rounded-md xs:rounded-lg transition-all capitalize group ${
                        t.slug === params.slug
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <span className={`font-medium text-sm xs:text-base ${t.slug === params.slug ? 'text-white' : 'text-slate-800 dark:text-gray-100 group-hover:text-indigo-600 dark:group-hover:text-indigo-400'}`}>
                          {t.title}
                        </span>
                        <span className={`text-xs xs:text-sm ${t.slug === params.slug ? 'text-white/80' : 'text-slate-500 dark:text-gray-400'}`}>
                          {t._count.NewsItem}
                        </span>
                      </div>
                    </Link>
